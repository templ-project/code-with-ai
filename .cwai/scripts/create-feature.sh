...
validate_github_token() {
    ...
    if ! gh auth status >/dev/null 2>&1; then
        error_exit "GitHub authentication failed. Please run 'gh auth login' or check your GH_TOKEN."
    fi
}
...
create_or_get_label() {
    ...
    log "🏷️ Creating label: $label_name"
...
create_new_feature() {
    ...
    if (( ${#labels[@]} > 0 )); then
        issue_number=$(create_github_issue "$feature_title" "$req" "${labels[@]}")
    else
        issue_number=$(create_github_issue "$feature_title" "$req")
    fi
...
update_existing_feature() {
    ...
    if (( ${#labels[@]} > 0 )); then
        update_github_issue "$issue_number" "$requirement" "${labels[@]}"
    else
        update_github_issue "$issue_number" "$requirement"
    fi
